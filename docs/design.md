...
| `OPNSENSE_APPLY_TIMEOUT`                                                                              | `120s`                         | Budget for the converge phases of one `ApplyChanges`; the trailing reconfigure runs on its own `OPNSENSE_RECONFIGURE_TIMEOUT`, so one apply can take up to their sum. Detached from the request context (section 6.2).     |
