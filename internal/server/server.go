...
func readyzHandler(probe ProbeFunc, ttl time.Duration) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		if err := cached.Check(r.Context()); err != nil {
			// Return a generic body. The probe error chain embeds the
			// firewall's host and a slice of its error response; the health
			// server listens on 0.0.0.0 with no auth, so echoing that to the
			// caller leaks internal topology. The cause is logged server-side
			// instead (see cachedProbe.Check). See #225.
