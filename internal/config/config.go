...
// not exposed here: the provider's own OPNSENSE_DOMAINS is what GetDomainFilter
// negotiates back, and it is a real constraint on what the firewall can hold
// rather than an echo of the operator's intent — the anti-pattern documented in
// sigs.k8s.io/external-dns/docs/contributing/sources-and-providers.md (PR #6249).
type Config struct {
	...
	// 180s exceeds OPNSENSE_APPLY_TIMEOUT plus OPNSENSE_RECONFIGURE_TIMEOUT
	// (165s by default) so a long apply and its reconfigure are never cut off
	// by the server.
	ServerWriteTimeout   time.Duration `env:"SERVER_WRITE_TIMEOUT"       envDefault:"180s"`
